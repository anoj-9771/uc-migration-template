...
CREATE    view [compliance].[vwAvetmissCourseOffering] as
	...
	select distinct 
		...
		convert(date, OfferingStartDate) AS START_DATE,
		DAY(OfferingStartDate) AS 'Start Date Day', 
		DATENAME(MONTH, OfferingStartDate) AS 'Start Date Month',
		YEAR(OfferingStartDate) AS 'Start Date Year',
		MONTH(OfferingStartDate) AS SORT_SD,
		convert(date, OfferingEndDate) AS END_DATE, 
		DAY(OfferingEndDate) AS 'End Date Day', 
		DATENAME(MONTH, OfferingEndDate) AS 'End Date Month',
		YEAR(OfferingEndDate) AS 'End Date Year',
		MONTH(OfferingEndDate) AS SORT_ED, 
		CASE AttendanceModeDescription 
		WHEN 'Full Time Connected Learning' THEN 'Full Time'
		WHEN 'Part Time Connected Learning' THEN 'Part Time'	
		WHEN 'Part Time Day Connected Learning' THEN 'Part Time Day'	
		WHEN 'Part Time Evening Connected Learning' THEN 'Part Time Evening'	
		ELSE AttendanceModeDescription 
		END AS 'Attendance Code', 
		--enrloc.LocationDescription AS 'Enrolment Location',
		...
		trim(OfferingCode) AS 'Offering Code', 
		TeachingSectionCode AS 'Offering Cost Centre Code',
		concat(OfferingEnrolmentLocation, '-', TeachingSectionDescription) AS 'Offering Code Centre', 
--		delloc.LocationName AS 'Offering Delivery Location', 
		co.OfferingDeliveryLocation as 'Offering Delivery Location',
		compliance.ProperCase(co.OfferingDeliveryModeDescription) AS 'Offering Delivery Mode',

		co.status AS 'Offering Status', 
		co.CourseCode AS 'Product Offering Code',
		CASE 
			WHEN CHARINDEX('TOL ', co.OfferingCode) > 0 THEN 'TOL in Offering Code'
			ELSE null
		END AS 'TOL in Offering Code',
		case 
	...
	from compliance.AvetmissCourseOffering co
		...
		--left join compliance.AvetmissLocation delloc on co.OfferingDeliveryLocation = delloc.LocationCode
		--	and delloc._RecordCurrent = 1 and delloc._RecordDeleted = 0
	where 1=1
