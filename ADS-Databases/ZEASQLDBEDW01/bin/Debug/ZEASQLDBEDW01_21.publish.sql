...
GO
PRINT N'Creating Table [edw].[reference_CommencingProgramIdentifierescfmt]...';
...
GO
PRINT N'Creating Table [edw].[reference_CommencingProgramIdentifierescnm]...';


GO
CREATE TABLE [edw].[reference_CommencingProgramIdentifierescnm] (
    [ESCNM]                   VARCHAR (100) NULL,
    [ESCNM_Description]       VARCHAR (100) NULL,
    [_DLRawZoneTimeStamp]     DATETIME2 (7) NULL,
    [_DLTrustedZoneTimeStamp] DATETIME2 (7) NULL,
    [_RecordStart]            DATETIME2 (7) NULL,
    [_RecordEnd]              DATETIME2 (7) NULL,
...
GO
PRINT N'Creating Table [edw].[reference_CommencingProgramIdentifierunitesc]...';
...
GO
PRINT N'Altering View [compliance].[vwAvetmissUnitEnrolment]...';
...
ALTER view [compliance].[vwAvetmissUnitEnrolment] as  



	select 
	
--Pre Adding CY to ID Fields--SUBSTRING(ue.ID_PC,5,999)  as ID_PC,--CONCAT(right(ue.ReportingYear, 2), ue.[InstituteId], ue.studentid) AS	'ID_PC',  --OK
ue.ID_PC as ID_PC,--CONCAT(right(ue.ReportingYear, 2), ue.[InstituteId], ue.studentid) AS	'ID_PC',  --OK
--cast
		--(
		--concat(
		--		right(ue.ReportingYear, 2)
		--		,ue.InstituteId
		--		,cast((convert(bigint,Ue.UnitOfferingID)) as varchar(20)) 
		--		)	
		--as VARCHAR
		--) as 'ID_UUIO',--OK
--Pre Adding CY to ID Fields--SUBSTRING (UE.ID_UUIO,3,999) as ID_UUIO,
UE.ID_UUIO as ID_UUIO,
--SUBSTRING(ce.ID_UIO,3,999) as ID_CUIO,
NULL as ID_CUIO,

--(substring(cast(YEAR(ue.UnitEnrolmentStartDate) as varchar(20)),3,2) + cast(ue.InstituteId as varchar(3)) + UnitDeliveryLocationCode + CAST(ue.[StudentId] as varchar) + ue.AvetmissCourseCode) as ID_CPU, 
--Pre Adding CY to ID Fields--SUBSTRING(ue.ID_CPU,5,999) as ID_CPU,
ue.ID_CPU as ID_CPU,
--CONCAT(right(ue.ReportingYear, 2), ue.[InstituteId], UnitOfferingEnrolmentID)	AS 	'ID_UPU', --- Rule: YY+INSTITUTE+UnitOfferingEnrolmentIDz
--Pre Adding CY to ID Fields--SUBSTRING(ue.ID_UPU,5,999) as ID_UPU,
ue.ID_UPU  as ID_UPU,
ue.[InstituteId] as INSTITUTE, 
[UnitDeliveryLocationCode] as LOCATION_CODE,
--YEAR([UnitEnrolmentStartDate]) AS ENROLMENT_YEAR,
...
--Jeremy requested that this field be one value before a certain report date and another after it. 

--AvetmissDeliveryModePre2018
--[AvetmissDeliveryMode]
--Case when ue.ReportingDate < '2020-05-24' --This need to be updated when Jeremy provides the date.
--THEN
		CASE 
...
----ELSE 
----		case when AvetmissDeliveryMode.AvetmissDeliveryModeCode IS NOT NULL
----		then
----		CONCAT(AvetmissDeliveryMode.AvetmissDeliveryModeCode , ' ' ,AvetmissDeliveryMode.AvetmissDeliveryModeDescription ) 
----		else null
----		end
--end
...
--NEED JEREMY TO CONFIRM THIS ONE.
CASE 
...
--		CASE
--			WHEN OUTCOME_ID_TRAINING IS NOT NULL THEN outcome_tafe.DESCRIPTION
--			ELSE NULL
--		END AS 'Outcome - TAFE', 
...
--CASE 
--	WHEN [UnitSponsorOrganisationCode] IS NOT NULL and [UnitSponsorOrganisationCode] not in ('INT_STDNT', 'MY_C_VFH', 'SEEK','SEEK_VFH')
--	--THEN CONCAT(SUBSTRING([UnitSponsorOrganisationCode], 4, 20), ' ', ue.SponsorDescription)
--	THEN Concat(UnitSponsorOrganisationCode, ' ', ue.SponsorDescription)
--	ELSE NULL 
...
--AvetmissLocation.LocationDescription AS 'Delivery Location - AVETMISS',
		case 
...
GO
PRINT N'Creating View [aas].[PartitionAllUnit]...';
...
create   VIEW [aas].[PartitionAllUnit] AS 
...
GO
PRINT N'Creating View [aas].[PartitionUnit]...';


GO
Create   view [aas].[PartitionUnit]
as
SELECT 
[ModelName]
,[PartitionName]
...
WHERE        (_RecordDeleted = 0) AND (_RecordCurrent = 1)
GO
PRINT N'Creating View [reference].[CommencingProgramIdentifierescfmt]...';


GO
CREATE view reference.CommencingProgramIdentifierescfmt
as 
Select [escfmt], [ESCNM], [_DLRawZoneTimeStamp], [_DLTrustedZoneTimeStamp], [_RecordStart], [_RecordEnd], [_RecordDeleted], [_RecordCurrent]
from [edw].[reference_CommencingProgramIdentifierescfmt]
GO
PRINT N'Creating View [reference].[CommencingProgramIdentifierescnm]...';
...
GO
PRINT N'Creating View [reference].[CommencingProgramIdentifierunitesc]...';


GO

CREATE view  reference.CommencingProgramIdentifierunitesc
AS 
Select [UNITesc], [ESCNM], [_DLRawZoneTimeStamp], [_DLTrustedZoneTimeStamp], [_RecordStart], [_RecordEnd], [_RecordDeleted], [_RecordCurrent]
From [edw].[reference_CommencingProgramIdentifierunitesc]
GO
