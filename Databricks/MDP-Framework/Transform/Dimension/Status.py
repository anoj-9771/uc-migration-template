...
def Transform():
    ...
    proc_type_df = GetTable(f"{get_table_namespace(f'{SOURCE}', 'crm_crmc_proc_type')}").select("objectTypeCode","userStatusProfile").dropDuplicates()
    object_df = spark.sql(
    ...
    status_usage_pref1 = spark.sql(f"""select distinct statusProfile as sp1 from {get_table_namespace(f'{SOURCE}', 'crm_0crm_srv_req_inci_h')} where statusProfile is not null""")
    status_usage_pref2 = spark.sql(f"""select distinct statusProfile as sp2 from {get_table_namespace(f'{SOURCE}', 'crm_0crm_sales_act_1')} where statusProfile is not null""")
    # ------------- JOINS ------------------ #
