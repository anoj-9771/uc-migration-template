...
df =  GetTable(f"{getEnv()}cleansed.crm.0crm_proc_type_text")

#     Linking table to connect proc_type to object_type
proc_type_df = GetTable(f"{getEnv()}cleansed.crm.crmc_proc_type").select("processTypeCode","objectTypeCode")
...
object_df = spark.sql(
        f"""SELECT objectTypeCode, objectTypeDescription FROM 
           ((SELECT objectTypeCode, objectTypeDescription, 1 AS record_number FROM {getEnv()}cleansed.crm.crmc_prt_otype WHERE objectTypeCode IN 
            (SELECT objectTypeCode FROM {getEnv()}cleansed.crm.crmc_prt_otype GROUP BY objectTypeCode HAVING COUNT(1) = 1))
           UNION
            (SELECT objectTypeCode, objectTypeDescription, ROW_NUMBER() OVER(PARTITION BY objectTypeCode ORDER BY extract_datetime DESC) AS record_number FROM {getEnv()}cleansed.crm.crmc_prt_otype WHERE objectTypeCode IN 
            (SELECT objectTypeCode FROM {getEnv()}cleansed.crm.crmc_prt_otype GROUP BY objectTypeCode HAVING COUNT(1) > 1) and DefaultBorType = 'X')
            ) where record_number = 1""")

process_usage_pref1 = spark.sql(f"""select distinct processTypeCode as pc1 from {getEnv()}cleansed.crm.0crm_srv_req_inci_h where processTypeCode is not null""")

process_usage_pref2 = spark.sql(f"""select distinct processTypeCode as pc2 from {getEnv()}cleansed.crm.0crm_sales_act_1  where processTypeCode is not null""")
...
#    display(df)
    #CleanSelf()
    Save(finaldf) #SaveWithCDF(finaldf, 'Full Load')
    #DisplaySelf()
pass
Transform()
