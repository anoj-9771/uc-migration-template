...
#     Linking table to connect proc_type to object_type
    proc_type_df = GetTable(f"{get_table_namespace(f'{SOURCE}', 'crm_crmc_proc_type')}").select("processTypeCode","objectTypeCode")
#    Select objectTypeDescription from crm_crmc_prt_otype such that there is only 1 record for each objectTypeCode 
    ...
    process_usage_pref1 = spark.sql(f"""select distinct processTypeCode as pc1 from {get_table_namespace(f'{SOURCE}', 'crm_0crm_srv_req_inci_h')} where processTypeCode is not null""")
    process_usage_pref2 = spark.sql(f"""select distinct processTypeCode as pc2 from {get_table_namespace(f'{SOURCE}', 'crm_0crm_sales_act_1')} where processTypeCode is not null""")
    # ------------- JOINS ------------------ #
