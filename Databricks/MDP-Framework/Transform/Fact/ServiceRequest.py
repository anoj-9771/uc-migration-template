...
def NSWWorkingDaysWithinRange(fromDates: pd.Series, toDates: pd.Series) -> pd.Series:
    ...
    for f, t in zip(fromDates, toDates):
        totalSeconds = 0
        while f <= t:
            if f.weekday() < 5 and f not in publicHolidays.values:
...
def Transform():
    ...
    .select("channelCode","communicationChannelSK")
    ...
    response_df = spark.sql(f"""select F.serviceRequestGUID,
                                       S.apptStartDatetime as respondByDateTime,
                                      CASE WHEN F.processTypeCode = 'ZCMP' AND S2.apptType = 'VALIDTO' THEN S2.apptStartDatetime 
                                           WHEN S2.apptType = 'ZRESPONDED' THEN  S2.apptStartDatetime ELSE NULL END as respondedDateTime,
                                      CASE WHEN S2.apptType = 'ZCLOSEDATE' THEN S2.apptStartDatetime ELSE NULL END as serviceRequestClosedDateTime,
                                      CASE WHEN S2.apptType = 'SRV_RREADY' THEN S2.apptStartDatetime ELSE NULL END as toDoByDateTime,
                                      COALESCE(F.requestEndDate, S2.apptStartDatetime) endDate,
             case when S2.apptStartDateTime is NULL then NULL else DateDiff(second,F.requestStartDate,COALESCE(S2.apptStartDatetime, F.requestEndDate))/(3600*24) end as interimResponseDays,             
             ...
             from hive_metastore.cleansed.crm_0crm_srv_req_inci_h F
             ...
             LEFT JOIN {SOURCE}.crm_scapptseg S2 on S2.ApplicationGUID = L.setGUID and  S2.apptType in ('VALIDTO','ZRESPONDED', 'ZCLOSEDATE', 'SRV_RREADY')           
            where L.setObjectType = '30'""") 

                 
    workingcalc_df = response_df.select("serviceRequestGUID", "respondedDateTime", "endDate")
    publicHolidaysPD = GetTable(f"{SOURCE}.datagov_australiapublicholidays").filter(expr("jurisdiction like '%NSW%'")) \
                                                                        .select('date').withColumnRenamed("date","holidayDate").toPandas()
                                                                        
    workingcalc_df = workingcalc_df.withColumn('interimResponseWorkingDays', workingDaysNSWVectorizedUDF(workingcalc_df['respondedDateTime'], workingcalc_df['endDate'])) \
    ...
    .join(channel_df,df.communicationChannelCode == channel_df.channelCode,"left") \
