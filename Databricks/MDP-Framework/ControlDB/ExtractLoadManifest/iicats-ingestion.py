...
df = spark.sql("""
    ...
    UNION 
    SELECT 'scx_point' SourceTableName, "TO_CHAR(M_DATE, ''YYYY/MM/DD HH24:MI:SS'')" WatermarkColumn, "select * from iicats.scx_point where lower(edw_export_config) = ''y''" SourceQuery, *, '{"deleteRecordsQuery" : "select * from {vwCleanseDataFrame} where pointInternalId in (select objectInternalId from {get_env()}cleansed.iicats.hierarchy_cnfgn where sourceRecordUpsertLogic = \\'\\'D\\'\\' and objestLevelNumber = 11 and _DLCleansedZoneTimeStamp > \\'\\'{lastLoadTimeStamp}\\'\\')"}' ExtendedProperties FROM _Base
    UNION 
    ...
    UNION 
    SELECT 'site_hierarchy' SourceTableName, "TO_CHAR(M_DATE, ''YYYY/MM/DD HH24:MI:SS'')" WatermarkColumn, "select * from iicats.site_hierarchy where lower(substr(scx_site_code,1,2)) <> ''tw'' and lower(edw_export_config) = ''y''" SourceQuery, *, '{"deleteRecordsQuery" : "select * from {vwCleanseDataFrame} where siteInternalId in (select objectInternalId from {get_env()}cleansed.iicats.hierarchy_cnfgn where sourceRecordUpsertLogic = \\'\\'D\\'\\' and objestLevelNumber = 8 and _DLCleansedZoneTimeStamp > \\'\\'{lastLoadTimeStamp}\\'\\')"}' ExtendedProperties FROM _Base    
    )    
