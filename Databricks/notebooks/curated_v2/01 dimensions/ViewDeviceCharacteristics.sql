...
WITH dateDriverNonDelete AS (
     ...
     effectiveDateRangesNonDelete AS (
         SELECT 
             ...
             archivingObjectsInternalId, 
             _effectiveFrom, 
             cast(COALESCE(TIMESTAMP((LEAD(_effectiveFrom,1) OVER(PARTITION BY deviceNumber,characteristicInternalId,classifiedEntityType,classTypeCode,archivingObjectsInternalId ORDER BY _effectiveFrom)) - INTERVAL 1 seconds), '9999-12-31') as timestamp) AS _effectiveTo
...
LEFT OUTER JOIN curated_v2.dimDeviceCharacteristics
  ON effectiveDateRanges.deviceNumber = dimDeviceCharacteristics.deviceNumber
        ...
        AND effectiveDateRanges._effectiveFrom <= dimDeviceCharacteristics._RecordEnd
        AND effectiveDateRanges._effectiveTo >= dimDeviceCharacteristics._RecordStart 
        AND dimDeviceCharacteristics._recordDeleted = 1
)
