...
def getBusinessPartnerGroupRelationship():
    ...
    isu0bpRelationsAttrDf  = spark.sql(f"select 'ISU' as sourceSystemCode, \
                                      ...
                                      where relationshipDirection = '1' and deletedFlag is null\
    ...
    dummyDimRecDf = spark.sql(f"select businessPartnerSK as dummyDimSK, 'dimBusinessPartner' as dimension from {ADS_DATABASE_CURATED}.dimBusinessPartner where businessPartnerNumber = '-1' \
                            union select businessPartnerGroupSK as dummyDimSK, 'dimBusinessPartnerGroup' as dimension from {ADS_DATABASE_CURATED}.dimBusinessPartnerGroup \
                                                                                                                                            where businessPartnerGroupNumber = '-1'")
    
    
    #2.Joins to derive SKs
    isu0bpRelationsAttrDf = isu0bpRelationsAttrDf.join(dim0bpartnerDf, (isu0bpRelationsAttrDf.businessPartnerNumber == dim0bpartnerDf.businessPartnerNumber), how="left") \
                    .select(isu0bpRelationsAttrDf['*'], dim0bpartnerDf['businessPartnerSK'])    
    
    isu0bpRelationsAttrDf = isu0bpRelationsAttrDf.join(dim0bpGroupDf, (isu0bpRelationsAttrDf.businessPartnerGroupNumber == dim0bpGroupDf.businessPartnerGroupNumber), how="left") \
