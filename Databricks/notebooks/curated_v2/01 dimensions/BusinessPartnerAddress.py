...
# MAGIC - Merge both CRM & ISU dataframes to create a "Master Business Partner Address" dataframe
...
df_isu_addr_attr = (
    spark.sql(f""" 
        SELECT 
            ...
            houseNumber                                                         AS housePrimaryNumber, -- RENAMED
            houseNumber2                                                        AS houseSupplementNumber, -- RENAMED
            ...
            concat( 
                ...
                coalesce(concat(houseNumber2, ', '), ''), 
                coalesce(concat(houseNumber, ', '), '') 
            ...
            CASE 
                WHEN countryShortName = 'AU' AND cityName IS NULL 
                THEN '' 
                ELSE stateCode 
            ...
            CASE 
                WHEN countryShortName = 'AU' AND cityName IS NULL 
                THEN '' 
                ELSE countryShortName 
            END                                                                 AS countryShortName, -- TRANSFORMATION
...
df_bp_addr_inner = (
    ...
    .join(
        ...
        [
            ...
            col("isu.addressNumber") == col("crm.addressNumber")
        ],
        how = 'inner'
    )
    # --- Select Columns --- #
...
df_bp_addr_crm_unique = (
    ...
    .join(
        ...
        [
            ...
            col("crm.addressNumber") == col("isu.addressNumber")
        ],
        how = 'leftanti'
    )
    .select("crm.*")
...
dummyDimRecDf = (
    spark.createDataFrame(
        [("-1", "-1", "Unknown")], 
        ["addressNumber", "businessPartnerNumber", "coName"]
    )
)
...
df_bp_addr_master = (
    # --- Union Tables ---#
    df_bp_addr_inner
    .unionByName(df_bp_addr_isu_unique)
...
# print('df_bp_addr_inner count:', df_bp_addr_inner.count())
# print('df_bp_addr_isu_unique count:', df_bp_addr_isu_unique.count())
...
TemplateEtlSCD(
    ...
    businessKey="addressNumber,businessPartnerNumber",
    schema=schema
)
