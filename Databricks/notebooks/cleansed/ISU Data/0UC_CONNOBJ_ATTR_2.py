...
df = spark.sql(f"WITH stage AS \
                      (Select *, ROW_NUMBER() OVER (PARTITION BY HAUS,COUNTRY ORDER BY _FileDateTimeStamp DESC, DI_SEQUENCE_NUMBER DESC, _DLRawZoneTimeStamp DESC) AS _RecordVersion FROM {delta_raw_tbl_name} WHERE _DLRawZoneTimestamp >= '{LastSuccessfulExecutionTS}') \
                           SELECT \
                                  ...
                                  (CASE WHEN con.LOEVM IS NULL THEN 'N' ELSE 'Y' END) as deletedFlag,\
                                  cast('1900-01-01' as TimeStamp) as _RecordStart, \
                                  cast('9999-12-31' as TimeStamp) as _RecordEnd, \
                                  (CASE WHEN con.LOEVM IS NULL THEN '0' ELSE '1' END) as _RecordDeleted, \
                                  '1' as _RecordCurrent, \
