...
-- MAGIC %md
-- MAGIC <h1>This notebook must run AFTER access_propertyTypeTimeslice</h1>
...
select 'thlot' as tbl, count(*)
from cleansed.access_z309_thlot
where date(rowSupersededTimeStamp) = '9999-12-31'
union all
select 'thmastrataplan' as tbl, count(*)
from cleansed.access_z309_thmastrataplan
where date(rowSupersededTimeStamp) = '9999-12-31'
union all
select 'thmastrataplan' as tbl, count(*)
from cleansed.access_z309_thstrataunits
where date(rowSupersededTimeStamp) = '9999-12-31'
-- and lotDetailsUpdatedDate = '9999-12-31'
...
select * from cleansed.access_z309_thmastrataplan
...
with t0 as (select propertyNumber, min(validFrom) as firstCreatedDate
    ...
    t1 as
        (--current table
        select 'current' as fromTable, a.masterPropertyNumber, 'SP' as planType, strataPlanNumber as planNumber, lotNumber, sectionNumber, lotTypeCode, lotType, portionNumber, subdivisionNumber, lotAreaSize, lotAreaSizeUnit, 
               coalesce(lotDetailsUpdatedDate, t0.firstCreatedDate) as validFrom, 1 as rn
        from   cleansed.access_z309_tlot a,
               t0
        where  a.propertyNumber = t0.propertyNumber
...
from tsPropertyLot
group by propertyNumber, planType
