...
df_cleansed = spark.sql(f"SELECT C_METE_TYPE AS meterTypeCode, \
		...
		case when F_SMAL_METE = 'Y' then true else false end AS isSmallMeter, \
		to_date(D_METE_TYPE_EFFE, 'yyyyMMdd') AS meterTypeEffectiveDate, \
		to_date(D_METE_TYPE_CANC, 'yyyyMMdd') AS meterTypeCancelledDate, \
		_RecordStart, \
		_RecordEnd, \
		_RecordDeleted, \
		_RecordCurrent \
...
newSchema = StructType([
	...
	StructField('isSmallMeter',BooleanType(),False),
	StructField('meterTypeEffectiveDate',DateType(),True),
	StructField('meterTypeCancelledDate',DateType(),True),
