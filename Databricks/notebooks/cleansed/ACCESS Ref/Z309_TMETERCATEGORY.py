...
import json
accessTable = 'Z309_TMETERCATEGORY'
...
df_cleansed = spark.sql("SELECT C_METE_CATE AS meterCategoryCode, \
		...
		to_date(D_METE_CATE_CANC, 'yyyyMMdd') AS meterCategoryCancelledDate, \
		_RecordStart, \
		_RecordEnd, \
		_RecordDeleted, \
		_RecordCurrent \
...
newSchema = StructType([
	StructField('meterCategoryCode',StringType(),False),
	StructField('meterCategory',StringType(),False),
	StructField('meterCategoryAbbreviation',StringType(),False),
	StructField('meterCategoryEffectiveDate',DateType(),True),
	StructField('meterCategoryCancelledDate',DateType(),True),
