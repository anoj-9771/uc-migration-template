...
df = spark.sql(f"WITH stage AS \
                      (Select *, ROW_NUMBER() OVER (PARTITION BY BCREASON ORDER BY _FileDateTimeStamp DESC, _DLRawZoneTimeStamp DESC) AS _RecordVersion FROM {delta_raw_tbl_name} WHERE _DLRawZoneTimestamp >= '{LastSuccessfulExecutionTS}') \
                           SELECT  \
                                case when BCREASON = 'na' then '' else BCREASON end as reversalReasonCode, \
                                TEXT40  as reversalReason, \
...
newSchema = StructType(
                            [
                            StructField("reversalReasonCode", StringType(), False),
