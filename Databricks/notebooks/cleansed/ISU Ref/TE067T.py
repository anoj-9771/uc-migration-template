...
df = spark.sql(f"WITH stage AS \
                      (Select *, ROW_NUMBER() OVER (PARTITION BY KONDIGR ORDER BY _FileDateTimeStamp DESC, _DLRawZoneTimeStamp DESC) AS _RecordVersion FROM {delta_raw_tbl_name} WHERE _DLRawZoneTimestamp >= '{LastSuccessfulExecutionTS}') \
                           SELECT  \
                                case when KONDIGR = 'na' then '' else KONDIGR end as rateFactGroupCode, \
                                TEXT30  as rateFactGroup, \
...
newSchema = StructType(
                            [
                            StructField("rateFactGroupCode", StringType(), False),
