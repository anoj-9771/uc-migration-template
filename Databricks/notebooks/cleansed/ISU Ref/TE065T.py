...
df = spark.sql(f"WITH stage AS \
                      (Select *, ROW_NUMBER() OVER (PARTITION BY SPARTYP,ZWKENN ORDER BY _FileDateTimeStamp DESC, _DLRawZoneTimeStamp DESC) AS _RecordVersion FROM {delta_raw_tbl_name} WHERE _DLRawZoneTimestamp >= '{LastSuccessfulExecutionTS}') \
                           SELECT  \
                                case when SPARTYP = 'na' then '' else SPARTYP end as divisionCategoryCode, \
                                case when ZWKENN = 'na' then '' else ZWKENN end as registerIdCode, \
                                ZWKTXT  as registerId, \
...
newSchema = StructType(
                            [
                            StructField("divisionCategoryCode", StringType(), False),
                            StructField("registerIdCode", StringType(), False),
