...
df = spark.sql(f"WITH stage AS \
                      (Select *, ROW_NUMBER() OVER (PARTITION BY SUPERIOR_PROP_TYPE ORDER BY _FileDateTimeStamp DESC, _DLRawZoneTimeStamp DESC) AS _RecordVersion FROM {delta_raw_tbl_name} WHERE _DLRawZoneTimestamp >= '{LastSuccessfulExecutionTS}') \
                           SELECT  \
                                case when SUPERIOR_PROP_TYPE = 'na' then '' else SUPERIOR_PROP_TYPE end as superiorPropertyTypeCode , \
