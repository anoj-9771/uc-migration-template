...
df = spark.sql(f"WITH stage AS \
                      (Select *, ROW_NUMBER() OVER (PARTITION BY FUNKLAS ORDER BY _FileDateTimeStamp DESC, _DLRawZoneTimeStamp DESC) AS _RecordVersion FROM {delta_raw_tbl_name} WHERE _DLRawZoneTimestamp >= '{LastSuccessfulExecutionTS}') \
                           SELECT \
                                case when FUNKLAS = 'na' then '' else FUNKLAS end as functionClassCode,\
...
newSchema = StructType([
                      StructField('functionClassCode',StringType(),False),
