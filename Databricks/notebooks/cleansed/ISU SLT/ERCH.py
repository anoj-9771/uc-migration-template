...
df = spark.sql(f"""
    WITH stage AS (
        ...
        FROM stage stg 
        LEFT OUTER JOIN {ADS_DATABASE_CLEANSED}.isu_0comp_code_text cc ON 
            cc.companyCode = stg.BUKRS 
        LEFT OUTER JOIN {ADS_DATABASE_CLEANSED}.isu_0division_text divText ON 
            divText.divisionCode = stg.SPARTE 
        LEFT OUTER JOIN {ADS_DATABASE_CLEANSED}.isu_0uc_portion_text pText ON 
            pText.portionNumber = stg.PORTION 
        LEFT OUTER JOIN {ADS_DATABASE_CLEANSED}.isu_0uc_hvorg_text hvorg_text ON 
            hvorg_text.maintransactionLineItemCode = stg.HVORG 
        LEFT OUTER JOIN {ADS_DATABASE_CLEANSED}.isu_dd07t dd07t ON 
        ...
        LEFT OUTER JOIN {ADS_DATABASE_CLEANSED}.isu_0fc_pymet_text pymet_text ON
...
display(
    spark.table("cleansed.isu_dd07t")
    .select("domainValueSingleUpperLimit")
    .where("domainName == 'ABRVORG'")
)
display(
    spark.table("raw.isu_erch")
    .select("ABRVORG")
    .dropDuplicates()
)
...
# display(
#     df
#     .select("billingTransactionCode", "billingTransaction")
#     .drop_duplicates()
# )
# display(
#     df
#     .select("mainTransactionCode", "mainTransaction")
#     .drop_duplicates()
# )
# display(
#     df
#     .groupBy("portionNumber", "portion")
#     .count()
# )
# display(
#     df
#     .groupBy("documentTypeCode", "documentType")
#     .count()
# )
# display(
#     df
#     .groupBy("suppressedBillingOrderTransactionCode", "suppressedBillingOrderTransaction")
#     .count()
# )
# display(
#     df
#     .groupBy("backbillingTypeCode", "backbillingType")
#     .count()
# )
# display(
#     df
#     .groupBy("reversalReasonCode", "reversalReason")
#     .count()
# )
# display(
#     df
#     .groupBy("billingDocumentWithoutInvoicingCode", "billingDocumentWithoutInvoicing")
#     .count()
# )
# display(
#     df
#     .groupBy("incomingPaymentMethodCode", "incomingPaymentMethod")
#     .count()
# )
