...
newSchema = StructType([
                          ...
                          StructField('amountPerAreaUnit', DoubleType(), True),
                          StructField('applicableIndicator', StringType(), True),
                          StructField('characteristicAmountArea', DoubleType(), True),
                          StructField('characteristicPercentage', DoubleType(), True),
...
cleansed_df = spark.sql(f"select distinct (case when INTRENO = 'na' then '' else INTRENO end) as INTRENO, (case when FIXFITCHARACT = 'na' then '' else FIXFITCHARACT end) as FIXFITCHARACT, ToValidDate(VALIDTO,'MANDATORY') as VALIDTO from ( \
...
cleansed_df.createOrReplaceTempView("isu_vibdcharact_deleted_records")
