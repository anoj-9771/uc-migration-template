...
import json
accessTable = 'Z309_TMETEREADTYPE'
...
df_cleansed = spark.sql("SELECT C_METE_READ_TYPE AS meterReadingTypeCode, \
		T_READ_TYPE_ABBR AS readingTypeAbbreviation, \
		initcap(T_METE_READ_TYPE) AS meterReadingType, \
		case when F_GENE_DEBI = 'Y' then true else false end AS isGenericDebit, \
		to_date(D_READ_TYPE_EFFE, 'yyyyMMdd') AS readingTypeEffectiveDate, \
		to_date(D_READ_TYPE_CANC, 'yyyyMMdd') AS readingTypeCancelledDate, \
		_RecordStart, \
		_RecordEnd, \
		_RecordDeleted, \
		_RecordCurrent \
	FROM CLEANSED.STG_ACCESS_Z309_TMETEREADTYPE \
         ")
...
newSchema = StructType([
	...
	StructField('readingTypeAbbreviation',StringType(),False),
	StructField('meterReadingType',StringType(),True),
	StructField('isGenericDebit',BooleanType(),False),
	StructField('readingTypeEffectiveDate',DateType(),True),
	StructField('readingTypeCancelledDate',DateType(),True),
