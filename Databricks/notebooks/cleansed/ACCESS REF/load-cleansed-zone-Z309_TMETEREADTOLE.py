...
import json
accessTable = 'Z309_TMETEREADTOLE'
...
df_cleansed = spark.sql("SELECT C_METE_READ_TOLE AS meterReadingToleranceCode, \
		...
		_RecordDeleted, \
		_RecordCurrent \
	FROM CLEANSED.STG_ACCESS_Z309_TMETEREADTOLE \
         ")
...
newSchema = StructType([
    StructField('meterReadingToleranceCode',StringType(),False),
	StructField('meterReadingTolerance',StringType(),False),
	StructField('readingToleranceEffectiveDate',DateType(),True),
	StructField('readingToleranceCancelledDate',DateType(),True),
