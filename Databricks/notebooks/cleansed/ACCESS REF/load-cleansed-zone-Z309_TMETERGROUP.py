...
df_cleansed = spark.sql("SELECT C_METE_GROU AS meterGroupCode, \
		initcap(T_METE_GROU) AS meterGroup, \
		to_date(D_METE_GROU_EFFE, 'yyyyMMdd') AS meterGroupEffectiveDate, \
		to_date(D_METE_GROU_CANC, 'yyyyMMdd') AS meterGroupCancelledDate, \
		_RecordStart, \
		_RecordEnd, \
		_RecordDeleted, \
		_RecordCurrent \
	FROM CLEANSED.STG_ACCESS_Z309_TMETERGROUP \
         ")
...
newSchema = StructType([
	StructField('meterGroupCode',StringType(),False),
	StructField('meterGroup',StringType(),True),
	StructField('meterGroupEffectiveDate',DateType(),True),
	StructField('meterGroupCancelledDate',DateType(),True),
