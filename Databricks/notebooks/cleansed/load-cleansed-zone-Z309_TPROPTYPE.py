...
df_cleansed = spark.sql("SELECT C_PROP_TYPE AS propertyTypeCode, \
		...
		case when F_SLIC_VALI = 'Y' then true \
                                    else false \
        end AS isSLICValid, \
		case when F_ACCO_PROP_VALI = 'Y' then true \
                                    else false \
        end AS isAccountPropertyValid, \
		case when F_RHLC_LIAB = 'Y' then true \
                               else false \
        end AS isRouseHillLandChargeLiable, \
		to_date(D_SUPE_PROP_EFFE, 'yyyyMMdd') AS propertyTypeEffectiveDate, \
		to_date(D_SUPE_PROP_CANC, 'yyyyMMdd') AS propertyTypeCancelledDate, \
        _RecordStart, \
        _RecordEnd, \
        _RecordDeleted, \
		_RecordCurrent \
...
newSchema = StructType([
	...
	StructField('propertyTypeAbbreviation',StringType(),False),
    StructField('propertyType',StringType(),False),
    StructField('isFlatValid',BooleanType(),False),
    StructField('isSLICValid',BooleanType(),False),
    StructField('isAccountPropertyValid',BooleanType(),False),
    StructField('isRouseHillLandChargeLiable',BooleanType(),False),
	StructField('propertyTypeEffectiveDate',DateType(),True),
	StructField('propertyTypeCancelledDate',DateType(),True),
