...
def getContract():
    ...
    df = spark.sql(f"with contract as ( \
                   select  co.contractId, \
                             ...
                             ca.applicationArea, \
                             co.installationId, \
                             row_number() OVER (PARTITION BY co.contractId,coh.validToDate ORDER BY coh.validToDate desc) as rank \
                   ...
                   select * from contract where rank=1 \
                     ").drop("rank")
