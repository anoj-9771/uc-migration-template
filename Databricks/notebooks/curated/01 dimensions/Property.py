...
#                     from   ISU""")
    df = sapisuDf
    dummyDimRecDf = spark.sql(f"""select waterNetworkSK as dummyDimSK, 'dimWaterNetwork_drinkingWater' as dimension from {ADS_DATABASE_CURATED}.dimWaterNetwork 
                          where supplyZone='Unknown' and  pressureArea='Unknown' 
                          union select waterNetworkSK as dummyDimSK, 'dimWaterNetwork_recycledWater' as dimension from {ADS_DATABASE_CURATED}.dimWaterNetwork 
...
curnt_table = f'{ADS_DATABASE_CURATED}.dimProperty'
