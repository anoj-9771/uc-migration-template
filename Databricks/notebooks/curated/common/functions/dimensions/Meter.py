...
#    display(isu0ucDeviceAttrDf)
    ...
    isu0ucDeviceHAttrDf  = spark.sql(f"select equipmentNumber as meterNumber, \
                                        logicalDeviceNumber as logicalMeterNumber, \
...
#     display(isu0ucDevCatAttrDf)
    isu0ucRegistAttrDf  = spark.sql(f"select equipmentNumber as meterNumber, \
                                        ...
                                        divisionCategory, \
                                        ToValidDate(a.validFromDate) as registerFromDate, \
                                        ToValidDate(a.validToDate,'MANDATORY') as registerToDate, \
                                        b.installationId, \
                                        ToValidDate(b.validFromDate) as reginstFromDate, \
                                        ToValidDate(b.validToDate,'MANDATORY') as reginstToDate \
                                      from {ADS_DATABASE_CLEANSED}.isu_0uc_regist_attr a \
                                      join {ADS_DATABASE_CLEANSED}.isu_0uc_reginst_str_attr b on a.logicalRegisterNumber = b.logicalRegisterNumber \
                                      where a._RecordCurrent = 1 and a._RecordDeleted = 0 and nvl(registerNotRelevantToBilling, ' ') <> 'X'")
	 
    print(f'{isu0ucRegistAttrDf.count():,} rows in isu0ucRegistAttrDf')
#     display(isu0ucRegistAttrDf)
                                    
#     isu0ucReginstStrAttrDf  = spark.sql(f"select logicalRegisterNumber, \
#                                       nvl(registerNotRelevantToBilling, ' ') as registerNotRelevantToBilling \
#                                       from {ADS_DATABASE_CLEANSED}.isu_0uc_reginst_str_attr \
#                                       where nvl(registerNotRelevantToBilling, ' ') <> 'X' \
#                                         and _RecordCurrent = 1 and _RecordDeleted = 0 ")
	 
#     print(f'{isu0ucReginstStrAttrDf.count():,} rows in isu0ucReginstStrAttrDf')
# #     display(isu0ucReginstStrAttrDf)                                    
...
#     display(df)
    df = df.join(isu0ucRegistAttrDf, (isu0ucRegistAttrDf.meterNumber == df.meterNumber) \
                                   & (df.validFromDate >= isu0ucRegistAttrDf.registerFromDate) \
                                   & (df.validToDate <= isu0ucRegistAttrDf.registerToDate) \
                                   & (df.validFromDate >= isu0ucRegistAttrDf.reginstFromDate) \
                                   & (df.validToDate <= isu0ucRegistAttrDf.reginstToDate), \
                                  how="leftouter") \
                            .drop(isu0ucRegistAttrDf.meterNumber)
    print(f'{df.count():,} rows after merge 3')
#     display(df)
    
#     df = df.join(isu0ucReginstStrAttrDf, (isu0ucReginstStrAttrDf.logicalRegisterNumber == df.logicalRegisterNumber), \
#                                   how="leftouter") \
#                             .drop(isu0ucReginstStrAttrDf.logicalRegisterNumber )
#     print(f'{df.count():,} rows after merge 4')
                                    
#     #re-order columns
    df = df.select('sourceSystemCode','meterNumber','installationID','validToDate','registerToDate','registerNumber','meterSerialNumber','logicalMeterNumber','validFromDate',
                    'materialNumber','usageMeterType','meterSize','waterType','meterCategoryCode','meterCategory',
...
#     dummyDimRecDf = spark.createDataFrame([ISUDummy, ACCESSDummy], df.columns)
    ...
    dummyDimRecDf = spark.createDataFrame([("ISU","-1","","2099-12-31","2099-12-31","","1900-01-01"),("ACCESS","-2","","2099-12-31","2099-12-31","","1900-01-01")], 
                                          ["sourceSystemCode", "meterNumber","installationID","validToDate","registerToDate","registerNumber","validFromDate"])    
...
#     #check key columns for null    
    df = df.withColumn("validToDate", when(df.validToDate.isNull(), "2099-12-31").otherwise(df.validToDate)) \
           ...
           .withColumn("installationID", when(df.installationID.isNull(), " ").otherwise(df.installationID))
...
#     display(df)
    ...
    newSchema = StructType([
                            ...
                            StructField('logicalMeterNumber', StringType(), True),
