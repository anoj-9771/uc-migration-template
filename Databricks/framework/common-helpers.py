...
def lookup_curated_namespace(env:str, current_database_name: str, current_table_name: str, csv_path:str) -> str:
    ...
    try:
        ...
        future_namespace['table_name'] = current_table_name.replace('dim', '') if 'dim' in current_table_name else current_table_name.replace('fact', '') if 'fact' in current_table_name else current_table_name.replace('brg', '') if 'brg' in current_table_name else current_table_name
...
def get_business_date(day_num: int):
    """return the xth business date of the current month."""
    return list(
        spark.sql(
                f"""
                select
        ...
        (
            ...
            *
            ...
            where
            calendarYear = year(current_date())
            and monthOfYear = month(current_date())
            and isHolidayFlag = 'N'
            and isWeekDayFlag = 'Y'
        )
