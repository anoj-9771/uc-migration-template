...
# MAGIC %sql
# MAGIC drop table if exists test.isu_0UC_TARIFTYP_TEXT
...
# MAGIC %sql
# MAGIC select distinct EXTRACT_DATETIME from test.isu_0UC_TARIFTYP_TEXT order by EXTRACT_DATETIME desc
...
# MAGIC TARIFTYP
# MAGIC ,TTYPBEZ
# MAGIC ,row_number() over (partition by TARIFTYP order by EXTRACT_DATETIME desc) as rn
# MAGIC from test.isu_0UC_TARIFTYP_TEXT
# MAGIC )where rn = 1
...
# MAGIC %sql
# MAGIC select count (*) as RecordCount, 'Target' as TableName from cleansed.isu_0UC_TARIFTYP_TEXT
# MAGIC union all
...
# MAGIC %sql
# MAGIC SELECT TARIFTYP, COUNT (*) as count
# MAGIC FROM cleansed.isu_0UC_TARIFTYP_TEXT
# MAGIC GROUP BY TARIFTYP
...
# MAGIC SELECT
# MAGIC *,
# MAGIC row_number() OVER(PARTITION BY TARIFTYP order by TARIFTYP) as rn
# MAGIC FROM  cleansed.isu_0UC_TARIFTYP_TEXT
