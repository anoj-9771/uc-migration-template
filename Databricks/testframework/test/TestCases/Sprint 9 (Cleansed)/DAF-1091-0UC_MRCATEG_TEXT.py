...
# MAGIC %sql
# MAGIC drop table if exists test.isu_0UC_MRCATEG_TEXT
...
# MAGIC %sql
# MAGIC select distinct EXTRACT_DATETIME from test.isu_0UC_MRCATEG_TEXT order by EXTRACT_DATETIME desc
...
# MAGIC ,row_number() over (partition by ABLESART order by ABLESART) as rn
# MAGIC from test.isu_0UC_MRCATEG_TEXT
# MAGIC )where rn = 1
...
# MAGIC %sql
# MAGIC select count (*) as RecordCount, 'Target' as TableName from cleansed.isu_0UC_MRCATEG_TEXT
# MAGIC union all
...
# MAGIC FROM cleansed.isu_0UC_MRCATEG_TEXT
