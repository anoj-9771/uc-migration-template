...
# MAGIC %sql
# MAGIC drop table if exists test.isu_0UC_OPERAND_TEXT
...
# MAGIC %sql
# MAGIC select distinct EXTRACT_DATETIME from test.isu_0UC_OPERAND_TEXT order by EXTRACT_DATETIME desc
...
# MAGIC ,row_number() over (partition by OPERAND order by OPERAND) as rn
# MAGIC from test.isu_0UC_OPERAND_TEXT
# MAGIC )where rn = 1
...
# MAGIC %sql
# MAGIC select count (*) as RecordCount, 'Target' as TableName from cleansed.isu_0UC_OPERAND_TEXT
# MAGIC union all
...
# MAGIC %sql
# MAGIC SELECT operandCode, COUNT (*) as count
# MAGIC FROM cleansed.isu_0UC_OPERAND_TEXT
# MAGIC GROUP BY operandCode
...
# MAGIC SELECT
# MAGIC *,
# MAGIC row_number() OVER(PARTITION BY operandCode order by operandCode) as rn
# MAGIC FROM  cleansed.isu_0UC_OPERAND_TEXT
