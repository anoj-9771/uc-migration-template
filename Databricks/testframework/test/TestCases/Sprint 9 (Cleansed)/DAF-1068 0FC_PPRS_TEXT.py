...
# MAGIC %sql
# MAGIC drop table if exists test.isu_0FC_PPRS_TEXT
...
# MAGIC %sql
# MAGIC select distinct EXTRACT_DATETIME from test.isu_0FC_PPRS_TEXT order by EXTRACT_DATETIME desc
...
# MAGIC ,promiseToPayReason
# MAGIC from (
# MAGIC select
# MAGIC PPRSC as promiseToPayReasonCode
# MAGIC ,TXT50 as promiseToPayReason
# MAGIC ,row_number() over (partition by PPRSC order by extract_datetime) rn
# MAGIC from test.isu_0FC_PPRS_TEXT) where rn = 1

# COMMAND ----------

# DBTITLE 1,[Verification] Count Check
# MAGIC %sql
# MAGIC select count (*) as RecordCount, 'Target' as TableName from cleansed.isu_0FC_PPRS_TEXT
# MAGIC union all
...
# MAGIC %sql
# MAGIC SELECT promiseToPayReasonCode, COUNT (*) as count
# MAGIC FROM cleansed.isu_0FC_PPRS_TEXT
# MAGIC GROUP BY promiseToPayReasonCode
...
# MAGIC SELECT
# MAGIC *,
# MAGIC row_number() OVER(PARTITION BY promiseToPayReasonCode order by promiseToPayReasonCode) as rn
# MAGIC FROM  cleansed.isu_0FC_PPRS_TEXT
...
# MAGIC ,promiseToPayReason
# MAGIC from
# MAGIC cleansed.isu_0FC_PPRS_TEXT
