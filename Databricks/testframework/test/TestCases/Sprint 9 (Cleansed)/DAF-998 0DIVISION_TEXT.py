...
# MAGIC %sql
# MAGIC drop table if exists test.isu_0DIVISION_TEXT
...
# MAGIC %sql
# MAGIC select distinct EXTRACT_DATETIME from test.isu_0division_text order by EXTRACT_DATETIME desc
...
# MAGIC ,row_number() over (partition by SPART order by extract_datetime) rn
# MAGIC from test.isu_0division_text) where rn = 1

# COMMAND ----------

# DBTITLE 1,[Verification] Count
# MAGIC %sql
# MAGIC select count (*) as RecordCount, 'Target' as TableName from cleansed.isu_0division_text
# MAGIC union all
...
# MAGIC %sql
# MAGIC SELECT divisionCode, COUNT (*) as count
# MAGIC FROM cleansed.isu_0division_text
# MAGIC GROUP BY divisionCode
...
# MAGIC SELECT
# MAGIC *,
# MAGIC row_number() OVER(PARTITION BY divisionCode order by divisionCode) as rn
# MAGIC FROM  cleansed.isu_0division_text
...
# MAGIC ,division
# MAGIC from
# MAGIC cleansed.isu_0division_text
