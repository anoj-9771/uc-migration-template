...
# MAGIC %sql
# MAGIC drop table if exists test.isu_0UC_VKTYP_TEXT
...
# MAGIC %sql
# MAGIC select distinct EXTRACT_DATETIME from test.isu_0UC_VKTYP_TEXT order by EXTRACT_DATETIME desc
...
# MAGIC ,TEXT as contractAccountCategory
# MAGIC ,VKTYP as contractACcountCategoryCode
# MAGIC ,row_number() over (partition by APPLK, VKTYP order by EXTRACT_DATETIME desc) as rn
# MAGIC from test.isu_0UC_VKTYP_TEXT
# MAGIC )where rn = 1
...
# MAGIC %sql
# MAGIC select count (*) as RecordCount, 'Target' as TableName from cleansed.isu_0UC_VKTYP_TEXT
# MAGIC union all
...
# MAGIC %sql
# MAGIC SELECT applicationArea,contractACcountCategoryCode, COUNT (*) as count
# MAGIC FROM cleansed.isu_0UC_VKTYP_TEXT
# MAGIC GROUP BY applicationArea,contractACcountCategoryCode
...
# MAGIC SELECT
# MAGIC *,
# MAGIC row_number() OVER(PARTITION BY applicationArea,contractACcountCategoryCode order by applicationArea,contractACcountCategoryCode) as rn
# MAGIC FROM  cleansed.isu_0UC_VKTYP_TEXT
