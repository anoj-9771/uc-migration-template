...
# MAGIC %sql
# MAGIC drop table if exists test.isu_icawriteoffrsnt
...
# MAGIC %sql
# MAGIC select distinct EXTRACT_DATETIME from test.isu_icawriteoffrsnt order by EXTRACT_DATETIME desc
...
# MAGIC ,writeOffReason
# MAGIC  from (select
# MAGIC CAWRITEOFFREASON as writeOffReasonCode
# MAGIC ,CAWRITEOFFREASONNAME as writeOffReason
# MAGIC ,row_number() over (partition by CAWRITEOFFREASON order by EXTRACT_DATETIME desc) as rn
# MAGIC from test.isu_icawriteoffrsnt)
# MAGIC where rn = 1
...
# MAGIC %sql
# MAGIC select count (*) as RecordCount, 'Target' as TableName from cleansed.isu_icawriteoffrsnt
# MAGIC union all
...
# MAGIC SELECT
# MAGIC *,
# MAGIC row_number() OVER(PARTITION BY writeOffReasonCode order by writeOffReasonCode) as rn
# MAGIC FROM  cleansed.isu_icawriteoffrsnt
# MAGIC ) where rn > 1
...
# DBTITLE 1,[Verification] Duplicate Checks
# MAGIC %sql
# MAGIC SELECT writeOffReasonCode, COUNT (*) as count
# MAGIC FROM cleansed.isu_icawriteoffrsnt
# MAGIC GROUP BY writeOffReasonCode
...
# MAGIC ,writeOffReason
# MAGIC from
# MAGIC cleansed.isu_icawriteoffrsnt
