...
# MAGIC %sql
# MAGIC drop table if exists test.isu_esendcontrolt
...
# MAGIC ,row_number() over (partition by SENDCONTROL order by EXTRACT_DATETIME desc) as rn
# MAGIC FROM
# MAGIC test.isu_esendcontrolt  --WHERE language = 'E'
# MAGIC ) a where a.rn = 1

# COMMAND ----------

lakedf = spark.sql("select * from cleansed.isu_esendcontrolt")
...
# MAGIC %sql
# MAGIC select count (*) as RecordCount, 'Target' as TableName from cleansed.isu_esendcontrolt
# MAGIC union all
...
# MAGIC ,row_number() over (partition by SENDCONTROL order by EXTRACT_DATETIME desc) as rn
# MAGIC FROM
# MAGIC test.isu_esendcontrolt --WHERE SPRAS = 'E'
# MAGIC ) a where a.rn = 1)
...
# MAGIC %sql
# MAGIC SELECT dispatchControlCode,dispatchControlDescription, COUNT (*) as count
# MAGIC FROM cleansed.isu_esendcontrolt
# MAGIC GROUP BY dispatchControlCode,dispatchControlDescription
...
# MAGIC SELECT
# MAGIC *,
# MAGIC row_number() OVER(PARTITION BY dispatchControlCode order by dispatchControlCode) as rn
# MAGIC FROM  cleansed.isu_esendcontrolt
# MAGIC )a where a.rn > 1
...
# MAGIC ,row_number() over (partition by SENDCONTROL order by EXTRACT_DATETIME desc) as rn
# MAGIC FROM
# MAGIC test.isu_esendcontrolt) a where a.rn = 1
# MAGIC except
# MAGIC select
# MAGIC dispatchControlCode
# MAGIC ,dispatchControlDescription
# MAGIC from
# MAGIC cleansed.isu_esendcontrolt
