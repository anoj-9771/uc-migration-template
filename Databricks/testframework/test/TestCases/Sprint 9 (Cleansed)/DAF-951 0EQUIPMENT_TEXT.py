...
# MAGIC %sql
# MAGIC drop table if exists test.isu_0EQUIPMENT_TEXT
...
# MAGIC %sql
# MAGIC select distinct EXTRACT_DATETIME from test.isu_0equipment_text order by EXTRACT_DATETIME desc
...
# MAGIC ,row_number() over (partition by EQUNR,DATETO  order by EXTRACT_DATETIME desc) as rn
# MAGIC from test.isu_0equipment_text -- WHERE LANGU = 'E'
...
# MAGIC %sql
# MAGIC select count (*) as RecordCount, 'Target' as TableName from cleansed.isu_0equipment_text
# MAGIC union all
...
# MAGIC %sql
# MAGIC SELECT equipmentNumber,validToDate, COUNT (*) as count
# MAGIC FROM cleansed.isu_0equipment_text
# MAGIC GROUP BY equipmentNumber,validToDate
...
# MAGIC SELECT
# MAGIC *,
# MAGIC row_number() OVER(PARTITION BY equipmentNumber,validToDate order by equipmentNumber,validToDate) as rn
# MAGIC FROM  cleansed.isu_0equipment_text
# MAGIC )a where a.rn > 1
...
# MAGIC from
# MAGIC cleansed.isu_0equipment_text
