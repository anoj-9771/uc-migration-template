...
# MAGIC %sql
# MAGIC drop table if exists test.0BP_CAT_TEXT
...
# MAGIC %sql
# MAGIC select * from test.0bp_cat_text
...
# MAGIC %sql
# MAGIC select
# MAGIC businessPartnerCategoyCode
# MAGIC ,businessPartnerCategoy 
# MAGIC from(select KEY1 as businessPartnerCategoyCode
# MAGIC ,TXTLG as businessPartnerCategoy
# MAGIC ,row_number() over (partition by businessPartnerCategoyCode order by EXTRACT_DATETIME desc) as rn
# MAGIC from source WHERE LANGU = 'E') a where a.rn = 1
...
# MAGIC SELECT
# MAGIC *,
# MAGIC row_number() OVER(PARTITION BY businessPartnerCategoyCode order by validFromDate) as rn
