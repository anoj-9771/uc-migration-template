...
# MAGIC %sql
# MAGIC drop table if exists test.isu_0fc_pprsw_text
...
# MAGIC %sql
# MAGIC select distinct EXTRACT_DATETIME from test.isu_0fc_pprsw_text order by EXTRACT_DATETIME desc
...
# MAGIC withdrawalReason
# MAGIC from (
# MAGIC select
# MAGIC PPRSW as withdrawalReasonCode
# MAGIC ,TXT50 as withdrawalReason
# MAGIC ,row_number() over (partition by PPRSW order by extract_datetime) rn
# MAGIC from test.isu_0fc_pprsw_text) where rn = 1

# COMMAND ----------

# DBTITLE 1,[Verification] Count
# MAGIC %sql
# MAGIC select count (*) as RecordCount, 'Target' as TableName from cleansed.isu_0fc_pprsw_text
# MAGIC union all
...
# MAGIC %sql
# MAGIC SELECT withdrawalReasonCode,withdrawalReason, COUNT (*) as count
# MAGIC FROM cleansed.isu_0fc_pprsw_text
# MAGIC GROUP BY withdrawalReasonCode,withdrawalReason
...
# MAGIC SELECT
# MAGIC *,
# MAGIC row_number() OVER(PARTITION BY withdrawalReasonCode order by withdrawalReasonCode) as rn
# MAGIC FROM  cleansed.isu_0fc_pprsw_text
...
# MAGIC withdrawalReason
# MAGIC from
# MAGIC cleansed.isu_0fc_pprsw_text
