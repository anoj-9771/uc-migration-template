...
# MAGIC %sql
# MAGIC drop table if exists test.isu_0fc_ppsta_text
...
# MAGIC %sql
# MAGIC select distinct EXTRACT_DATETIME from test.isu_0fc_ppsta_text order by EXTRACT_DATETIME desc
...
# MAGIC %sql
# MAGIC select 
# MAGIC promiseToPayStatusCode,
# MAGIC promiseToPayStatus
# MAGIC from (
# MAGIC select
# MAGIC KEY1 as promiseToPayStatusCode
# MAGIC ,TXTLG as promiseToPayStatus
# MAGIC ,row_number() over (partition by KEY1 order by extract_datetime) rn
# MAGIC from test.isu_0fc_ppsta_text) where rn = 1

# COMMAND ----------

# DBTITLE 1,[Verification] Count
# MAGIC %sql
# MAGIC select count (*) as RecordCount, 'Target' as TableName from cleansed.isu_0fc_ppsta_text
# MAGIC union all
...
# MAGIC %sql
# MAGIC SELECT promiseToPayStatusCode,promiseToPayStatus, COUNT (*) as count
# MAGIC FROM cleansed.isu_0fc_ppsta_text
# MAGIC GROUP BY promiseToPayStatusCode,promiseToPayStatus
...
# MAGIC SELECT
# MAGIC *,
# MAGIC row_number() OVER(PARTITION BY promiseToPayStatusCode order by promiseToPayStatusCode) as rn
# MAGIC FROM  cleansed.isu_0fc_ppsta_text
...
# MAGIC %sql
# MAGIC select 
# MAGIC case when promiseToPayStatusCode is null then '' else promiseToPayStatusCode end as promiseToPayStatusCode,
# MAGIC promiseToPayStatus
# MAGIC from (
# MAGIC select
# MAGIC KEY1 as promiseToPayStatusCode
...
# MAGIC promiseToPayStatus
# MAGIC from
# MAGIC cleansed.isu_0fc_ppsta_text
