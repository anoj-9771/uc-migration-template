...
# MAGIC %sql
# MAGIC drop table if exists test.isu_0uc_tvorg_text
...
# MAGIC %sql
# MAGIC select distinct EXTRACT_DATETIME from test.isu_0uc_tvorg_text order by EXTRACT_DATETIME desc
...
# MAGIC from test.isu_0uc_tvorg_text)
# MAGIC where rn = 1
...
# MAGIC %sql
# MAGIC select count (*) as RecordCount, 'Target' as TableName from cleansed.isu_0uc_tvorg_text
# MAGIC union all
...
# MAGIC SELECT
# MAGIC *,
# MAGIC row_number() OVER(PARTITION BY applicationArea,mainTransactionLineItemCode,subtransactionLineItemCode order by applicationArea,mainTransactionLineItemCode,subtransactionLineItemCode) as rn
# MAGIC FROM  cleansed.isu_0uc_tvorg_text
# MAGIC ) where rn > 1
...
# DBTITLE 1,[Verification] Duplicate Checks
# MAGIC %sql
# MAGIC SELECT applicationArea,mainTransactionLineItemCode,subtransactionLineItemCode, COUNT (*) as count
# MAGIC FROM cleansed.isu_0uc_tvorg_text
# MAGIC GROUP BY applicationArea,mainTransactionLineItemCode,subtransactionLineItemCode
...
# MAGIC APPLK as applicationArea
# MAGIC ,HVORG as mainTransactionLineItemCode
# MAGIC ,CASE WHEN TVORG is null THEN '' else TVORG END AS subtransactionLineItemCode
# MAGIC ,TXT30 as subtransaction
# MAGIC ,row_number() over (partition by APPLK,HVORG,TVORG order by EXTRACT_DATETIME desc) as rn
# MAGIC from test.isu_0uc_tvorg_text)
# MAGIC where rn = 1
...
# MAGIC ,mainTransactionLineItemCode
# MAGIC ,subtransactionLineItemCode
# MAGIC ,subtransaction
# MAGIC from
# MAGIC cleansed.isu_0uc_tvorg_text
