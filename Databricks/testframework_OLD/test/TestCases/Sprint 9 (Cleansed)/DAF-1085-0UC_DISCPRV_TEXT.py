...
# MAGIC %sql
# MAGIC drop table if exists test.isu_0UC_DISCPRV_TEXT
...
# MAGIC %sql
# MAGIC select distinct EXTRACT_DATETIME from test.isu_0UC_DISCPRV_TEXT order by EXTRACT_DATETIME desc
...
# MAGIC ,row_number() over (partition by DISCPROCV order by DISCPROCV) as rn
# MAGIC from test.isu_0UC_DISCPRV_TEXT
# MAGIC )where rn = 1
...
# MAGIC %sql
# MAGIC select count (*) as RecordCount, 'Target' as TableName from cleansed.isu_0UC_DISCPRV_TEXT
# MAGIC union all
...
# MAGIC %sql
# MAGIC SELECT processingVariantCode, COUNT (*) as count
# MAGIC FROM cleansed.isu_0UC_DISCPRV_TEXT
# MAGIC GROUP BY processingVariantCode
...
# MAGIC SELECT
# MAGIC *,
# MAGIC row_number() OVER(PARTITION BY processingVariantCode order by processingVariantCode) as rn
# MAGIC FROM  cleansed.isu_0UC_DISCPRV_TEXT
