...
# MAGIC %sql
# MAGIC drop table if exists test.isu_0FC_PPCAT_TEXT
...
# MAGIC %sql
# MAGIC select distinct EXTRACT_DATETIME from test.isu_0FC_PPCAT_TEXT order by EXTRACT_DATETIME desc
...
# MAGIC ,promiseToPayCategory
# MAGIC from (
# MAGIC select
# MAGIC PPCAT as promiseToPayCategoryCode
# MAGIC ,TXT50 as promiseToPayCategory
# MAGIC ,row_number() over (partition by PPCAT order by PPCAT) rn
# MAGIC from test.isu_0fc_ppcat_text where SPRAS = 'E') where rn = 1

# COMMAND ----------

# DBTITLE 1,[Verification] Count Checks
# MAGIC %sql
# MAGIC select count (*) as RecordCount, 'Target' as TableName from cleansed.isu_0fc_ppcat_text
# MAGIC union all
...
# MAGIC %sql
# MAGIC SELECT promiseToPayCategoryCode, COUNT (*) as count
# MAGIC FROM cleansed.isu_0fc_ppcat_text
# MAGIC GROUP BY promiseToPayCategoryCode
...
# MAGIC SELECT
# MAGIC *,
# MAGIC row_number() OVER(PARTITION BY promiseToPayCategoryCode order by promiseToPayCategoryCode) as rn
# MAGIC FROM  cleansed.isu_0fc_ppcat_text
...
# MAGIC ,promiseToPayCategory
# MAGIC from
# MAGIC cleansed.isu_0fc_ppcat_text
