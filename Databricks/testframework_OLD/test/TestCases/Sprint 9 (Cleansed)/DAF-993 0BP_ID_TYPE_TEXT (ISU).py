...
# MAGIC %sql
# MAGIC drop table if exists test.isu_0BP_ID_TYPE_TEXT
...
sourcedf = spark.sql(f"select * from test.isu_0bp_id_type_text")
display(sourcedf)

# COMMAND ----------

targetdf = spark.sql(f"select * from cleansed.isu_0bp_id_type_text")
display(targetdf)
...
# MAGIC from (
# MAGIC select *,
# MAGIC row_number () over (partition by type order by extract_datetime desc) rn
# MAGIC from test.isu_0bp_id_type_text) where rn = 1

# COMMAND ----------

# DBTITLE 1,[Verification] Count
# MAGIC %sql
# MAGIC select count (*) as RecordCount, 'Target' as TableName from cleansed.isu_0bp_id_type_Text
# MAGIC union all
...
# MAGIC %sql
# MAGIC SELECT identificationTypeCode, COUNT (*) as count
# MAGIC FROM cleansed.isu_0bp_id_type_text
...
# MAGIC from
# MAGIC cleansed.isu_0bp_id_type_text
