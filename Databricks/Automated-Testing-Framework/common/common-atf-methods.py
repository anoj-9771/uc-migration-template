...
def SchemaChk():
    ...
    if count > 0: 
        ...
        display(mappingDf)
        print('Schema Check mismatches - Columns in Target that are not in Mapping (Target-Mapping):')
        display(diff1)
        print('Schema Check mismatches - Columns in Mapping that are not in Target (Mapping-Target):')
        display(diff2)
...
def DuplicateKeysChk():
    ...
    if keyColumns.strip() == "":
        ...
        elif GetDatabaseName().upper() == 'CURATED' or GetDatabaseName().upper() == "CURATED_V2" or GetDatabaseName().upper() == "CURATED_V3":
