...
GO
PRINT N'Rename refactoring operation with key e3fd0fd2-dc20-432b-b2d3-a503f45b5b0d, 5474588e-4ad7-4e97-9aa8-350a7bd63946 is skipped, element [CTL].[ControlDataLoadTypes].[IsDelta] (SqlSimpleColumn) will not be renamed to DeltaExtract';


GO
PRINT N'Rename refactoring operation with key 7adbd24a-2748-40ff-858c-7f0fb8835504, 3a43145e-b25c-463e-ae75-c9c37286eb27 is skipped, element [CTL].[ControlDataLoadTypes].[IsCDC] (SqlSimpleColumn) will not be renamed to CDCSource';
...
GO
PRINT N'Rename refactoring operation with key b725dc32-6263-46dd-be40-7e4bcca4d5cb is skipped, element [CTL].[ControlManifest].[StageRecordProcessed] (SqlSimpleColumn) will not be renamed to [ProcessedToTrustedZone]';
...
GO
PRINT N'Creating [CTL].[TaskExecutionLog]...';
...
GO
PRINT N'Creating [CTL].[ControlWatermark]...';
...
GO
PRINT N'Creating [CTL].[ControlTaskCommand]...';
...
GO
PRINT N'Creating [CTL].[ControlTargetColumns]...';
...
GO
PRINT N'Creating [CTL].[ControlTarget]...';
...
GO
PRINT N'Creating [CTL].[ControlStages]...';
...
GO
PRINT N'Creating [CTL].[ControlSourceColumns]...';
...
GO
PRINT N'Creating [CTL].[ControlSource]...';
...
GO
PRINT N'Creating [CTL].[ControlProjects]...';
...
GO
PRINT N'Creating [CTL].[Configuration]...';
...
GO
PRINT N'Creating [CTL].[ControlProjectSchedule]...';
...
GO
PRINT N'Creating [CTL].[ControlTasks]...';
...
GO
PRINT N'Creating [CTL].[ControlDataLoadTypes]...';
...
GO
PRINT N'Creating [CTL].[ControlTypes]...';
...
GO
PRINT N'Creating [CTL].[ControlManifest]...';
...
CREATE TABLE [CTL].[ControlManifest] (
    ...
    [ProcessedToTrustedZone] BIGINT         NULL,
...
GO
PRINT N'Creating [CTL].[BatchExecutionLog]...';
...
GO
PRINT N'Creating [CTL].[FK_TaskExecutionLog_BatchExecutionLog]...';


GO
ALTER TABLE [CTL].[TaskExecutionLog] WITH NOCHECK
...
GO
PRINT N'Creating [CTL].[FK_TaskExecutionLog_ControlTasks]...';
...
GO
PRINT N'Creating [CTL].[FK_ControlTasks_ControlTarget]...';
...
GO
PRINT N'Creating [CTL].[vw_TaskExecutionDetails]...';
...
GO
PRINT N'Creating [CTL].[udf_GetWAustDateTime]...';
		...
		Else
		  BEGIN
			Insert Into CTL.TaskExecutionLog (BatchExecutionId, ControlTaskId, StartTime, ExecutionStatus) 
				Values (@BatchLogId, @TaskId, getdate(), 'In Progress')
...
Select distinct BatchExecutionId 
Into #tempBatch
From CTL.TaskExecutionLog
Where ControlTaskId in (select TaskId from CTL.ControlTasks where ProjectId = @projectId)
...
Where BatchExecutionLogId in (select BatchExecutionLogId from #tempBatch)
...
BEGIN
	...
	Update CTL.ControlWatermark
	   Set Watermarks = @Watermark
	 Where ControlSourceId = @SourceId
END
GO
PRINT N'Creating [CTL].[ClearErrorLogInProgress]...';


GO

CREATE PROCEDURE CTL.ClearErrorLogInProgress
AS
...
GO
PRINT N'Creating [CTL].[UpdateManifest]...';


GO
CREATE PROCEDURE [CTL].[UpdateManifest] (
...
--EXEC [CTL].[createTask] 'SalesOrders','Sales.Orders',1,'ADF-LS-SQLServer-WWI','','SalesOrders','WWI',2,'rawZone-rwdl-SasToken',1,1,1,'Select * From Sales.Orders'
--EXEC [CTL].[createTask] 'SalesOrders','WWI',2,'rawZone-rwdl-SasToken','databricks-Token|0813-134608-honed895','SalesOrders','WWI',4,'stageZone-rwdl-SasToken',2,1,8,'/build/DataProcessing/RawToStage-dp.py'
...
BEGIN
	...
	Begin Transaction
		...
		Insert Into CTL.ControlSource (SourceName, SourceTypeId, SourceLocation, LoadSource, SourceServer, Processor, BusinessKeyColumn) 
		Values(@SourceName, @SourceTypeId, @SourceLocation, 1, @SourceServer, @Processor, @BusinessKeyColumn)
...
*/
		Insert Into CTL.ControlTasks (TaskName, SourceId, TargetId, TruncateTarget, TaskEnabled, LoadLatestOnly, ExecuteSourceSQLasStoredProc, ControlStageId, ProjectId, ObjectGrain, DataLoadMode, TrackChanges)
		Values ('Copy ' + @SourceName, @SourceId, @TargetId, 0, 1, 0, 1, @StageId, @ProjectId, @Grain, @DataLoadMode, @TrackChangeFlag)
		...
		BEGIN
			...
			INSERT INTO CTL.ControlWatermark (ControlSourceId, SourceColumn, SourceSQL, Watermarks)
			VALUES (@TaskId, '__$start_lsn', '', @StartLSN)
		END
		...
		BEGIN
			...
			INSERT INTO CTL.ControlWatermark (ControlSourceId, SourceColumn, SourceSQL, Watermarks)
			VALUES (@TaskId, @WatermarkColumn, @WatermarkColumn, '2000-01-01 00:00:00')
		END
...
begin
	...
	IF @DeltaExtract = 1
	BEGIN
		set @blobfilename = @FileName + '_' + format([CTL].udf_GetNZDateTime(SYSDATETIME()),N'yyyy-MM-dd_HHMMss_fff') + '.' + @DataType
	END
	ELSE
	BEGIN
		set @blobfilename = @FileName + '_' + format([CTL].udf_GetNZDateTime(SYSDATETIME()),N'yyyy-MM-dd') + '.' + @DataType
...
Select ct.TaskId, Max(tel.EndTime) LastExecutionDate
 ...
 Where ct.TaskEnabled = 1
   And ct.ControlStageId = @StageId  
   And ct.ProjectId = @ProjectId
   And tel.ExecutionStatus In ('Success', 'Failure')
 Group By ct.TaskId
),
noExecutions As
(
...
CREATE PROC [CTL].[CreateSource] 
		...
		@StartStageName		 varchar(50),
		@EndStageName		 varchar(50),
		@SourceName			 varchar(100),
		@SourceObjectName	 varchar(500),
		@SourceType			 varchar(50),
		@DataLoadMode		 varchar(50),
		@SourceSecretName	 varchar(50),
		@DLRawSecret		 varchar(50),
		@DLStagedSecret		 varchar(50),
		@DBProcessor		 varchar(50),
		@StageDBSecret		 varchar(50),
		@DLRawSubFolder		 varchar(50),
		@DLRawType			 varchar(50),
		@DLStagedMainFolder	 varchar(50),
		@DLStagedSubFolder	 varchar(50),
		@DLStagedType		 varchar(50),
		@DLObjectGrain		 varchar(50),
		@SourceCommand		 varchar(5000),
		@DLRawtoStageCommand varchar(5000),
		@DLStagetoDBCommand  varchar(5000),
		@TargetObjectType	 varchar(50),
...
IF @DataLoadMode = 'INCREMENTAL' AND @BusinessKeyColumn = ''
	RAISERROR('Please specify a BusinessKeyColumn column when the data load mode is INCREMENTAL', 16, 1) 
...
--Updated by Rahul Agrawal on 29/-1/2020. Without these the stage tables are created on target database with the Soure Schema. 
--Also, the schema needs to exists beforehand. Having a single STG schema will help
set @stagingTable = 'STG' + '.' + @DLStagedSubFolder
--set @cdcFlag = case when @IsCDC = 'Yes' then 1 else 0 end
...
PRINT @DLRawSubFolder
PRINT @DLStagedMainFolder

	if @currentStage = 'Trusted to Staging'
		EXEC [CTL].[CreateTask] 
		@SourceName
		,@DLStagedMainFolder
		,@stageTypeId
...
IF (
	...
	BEGIN
		...
		INSERT [CTL].[ControlStages] ([ControlStageId], [StageSequence], [StageName]) VALUES (3, 300, N'Trusted to Staging')
